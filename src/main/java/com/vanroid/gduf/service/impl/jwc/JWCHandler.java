...
public class JWCHandler {
	...
	public String login(JwcInfo info) throws ClientProtocolException,
		...
		// 目的是获取ViewState
		String code = HttpClientUtils
				.get(null, GdufLinks.JWC_LOGIN, null, null).getBody();
		// 获取ViewState值
		String __VIEWSTATE = coursehtmlHandler.getVIEWSTATE(code);
		System.out.println("VIEWSTATE的值为：" + __VIEWSTATE);
		// 前一连接
		referer = "http://jwc.gduf.edu.cn/(1kr3xaag1lzthj45feqise55)/xs_main.aspx?xh="
				+ info.getXh();
		...
		HttpClientUtils.post(null, GdufLinks.JWC_LOGIN, null, params, "utf-8");
		String code2 = HttpClientUtils.get(null, referer, null, null).getBody();
	...
	public Course getCourseData(String xh, String xm, String year, int xq)
		...
		String v = getViewState(url);
		String code = getCourseCode(url, v, year, xq);
		List<ClassBean> list = coursehtmlHandler.execute(code);
	...
	public Grade getGrade(String xh, String xm, String year, int xq)
		...
		String v = getViewState(gradeurl);
		String code = getGradeCode(gradeurl, v, year, xq);
	 ...
	 * 获取成绩单的数据源码
	 */
	private String getGradeCode(String url, String v, String year, int xq)
			throws ClientProtocolException, IOException {
		...
		HttpClientUtils.post(null, url, headers, params, "utf-8");
		return code;
		/*
		 * List<NameValuePair> params = new ArrayList<NameValuePair>();
		 * params.add(new BasicNameValuePair("__EVENTARGUMENT", ""));
		 * params.add(new BasicNameValuePair("__EVENTTARGET", ""));
		 * params.add(new BasicNameValuePair("__VIEWSTATE", v)); params.add(new
		 * BasicNameValuePair("btn_xq", "学期成绩")); params.add(new
		 * BasicNameValuePair("ddlXN",year)); params.add(new
		 * BasicNameValuePair("ddlXQ",xq+"")); params.add(new
		 * BasicNameValuePair("ddl_kcxz","")); params.add(new
		 * BasicNameValuePair("hidLanguage",""));
		 */
		...
		// // httpRequest.setHeader("Content-Length", "1675");
		...
		// HttpHandler().getResponse(httpRequest);
		// //
		// System.out.println("状态码"+httpResponse.getStatusLine().getStatusCode());
		// if (httpResponse.getStatusLine().getStatusCode() == 200) {
		// code = HttpHandler.getResultString(httpResponse);
		// // System.out.println("成绩单的code" + code);
		// }
	...
	public String getViewState(String url) throws ClientProtocolException,
			IOException {
		...
		String code = HttpClientUtils.get(null, url, headers, null).getBody();
	...
	//
	// // 获取不同学期的课程表
	public String getCourseCode(String url, String __VIEWSTATE, String year,
		...
		// params.add(new BasicNameValuePair("xqd", xq + ""));*/
		//
		// httpRequest.setEntity(new UrlEncodedFormEntity(params, "utf-8"));
		// httpRequest.setHeader("Accept",
		// "text/html, application/xhtml+xml, */*");
		...
		// "Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.2; WOW64; Trident/6.0)");
		//
		// HttpResponse httpResponse = new HttpHandler()
